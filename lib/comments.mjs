...
function scriptKind(fileName) {
	...
	if (/\.(mts|cts|ts)$/.test(fileName)) return ts.ScriptKind.TS;
...
export function commentRanges(text, fileName) {
	...
	visit(sf);
	if (sf.endOfFileToken) add(ts.getLeadingCommentRanges(text, sf.endOfFileToken.pos));
...
export function commentBlocks(text, fileName, grammar = {}) {
	...
	for (const r of ranges) {
		...
		if (cur !== null) blocks.push(finalize(cur, text, starts, grammar));
