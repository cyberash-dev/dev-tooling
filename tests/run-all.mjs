...
check("R2 fires on change-narrative", rules("// renamed from Foo\nconst x = 1;\n").includes("R2"));
check("R5 fires on decorative", rules("// =====\nconst x = 1;\n").includes("R5"));
check(
	"R3 fires on usage snippet",
	rules("//   import x from 'y';\n//   export default x;\nconst z = 1;\n").includes("R3"),
);
check(
...
check(
	...
	!rules("// gatehouse:driver:DLT-025 disables a hook\nconst x = 1;\n").includes("R4"),
);
check(
...
check("--fix keeps the anchor line", fixed.includes("gatehouse:driver:DLT-025"));
check("--fix keeps the code", fixed.includes("const x = 1;"));
check("--fix is idempotent", fixText(fixed, "x.ts", ctx) === fixed);

const snippet = "//   import x from 'y';\n//   export default x;\nconst z = 1;\n";
check("--fix removes pure snippet", !fixText(snippet, "x.ts", ctx).includes("import x"));
...
check(
	...
	genericRules("// renamed from Foo; see gatehouse:driver:DLT-025\nconst x = 1;\n").includes("R2"),
);
check(
...
check(
	...
	!fixText("// gatehouse-local-driver:INV-999 stale\nconst x = 1;\n", "x.ts", ctx).includes(
		"INV-999",
	),
);
check(
...
check(
	...
	fixText("// gatehouse:driver:DLT-025 keep\nconst x = 1;\n", "x.ts", ctx).includes(
		"gatehouse:driver:DLT-025",
	),
);
...
process.stdout.write(`comment-lint self-test: ${passed} passed, ${failed} failed.\n`);
